...
static bool g_runtime_measure = false;
...
#if defined(_WIN32)
    // Never perform synchronous file I/O on the Windows MoE decode path.
    // mmap/RAM remains the fallback source.
    (void) key;
    (void) e;
    return fallback;
#endif
...
static void pool_fill(layer_tier & L, int k, int e) {
    ...
    if (g_runtime_measure) {
        ...
        g_runtime_fill_bytes += copied;
        g_runtime_fills++;

        if (g_runtime_fills <= 32) {
            TIER_LOG(
                "RUNTIME_FILL #%llu layer=%d expert=%d %.2f MiB %.3f ms\n",
                (unsigned long long) g_runtime_fills,
                L.il,
                e,
...
void init(const llama_model & model) {
    ...
    const size_t safety_buffer = 256ULL * 1024 * 1024;
