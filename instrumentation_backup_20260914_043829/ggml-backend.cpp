    ...
    // pass 5: split graph, find tensors that need to be copied
    {
        ...
        for (; i < graph->n_nodes; i++) {
            ...
            if (node_backend_id == cur_backend_id && split->n_inputs > 0) {
                for (int j = 0; j < GGML_MAX_SRC; j++) {
                    ...
                    if (src->buffer != NULL && src->buffer->usage == GGML_BACKEND_BUFFER_USAGE_WEIGHTS) {
                        ...
                        // Ornith/wackMall cold-MoE path:
                        // cold expert weights are handled explicitly by the
                        // scheduler compute path; do not create an extra
                        // backend split solely because the weight buffer is
                        // incompatible with the current backend.
                        const bool ornith_cold_moe =
                            node->op == GGML_OP_MUL_MAT_ID_COLD ||
                            node->op == GGML_OP_MOE_COLD;
            ...
            if (node_backend_id != cur_backend_id || need_new_split) {
                // ORNITH_SPLIT_DIAG: measurement only.
                // Do not change scheduler behavior here.
                {
                    const char * ornith_reason =
                        node_backend_id != cur_backend_id
                            ? "backend-change"
                            : "input-or-weight";

                    fprintf(stderr,
                        "ORNITH_SPLIT split=%d node=%d op=%s old_backend=%d new_backend=%d reason=%s inputs=%d\n",
                        i_split,
                        i,
